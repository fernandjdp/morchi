...
export default function Home() {
  ...
  const [sizeDrawerProduct, setSizeDrawerProduct] = useState<(typeof products)[number] | null>(null)
  ...
  const openSizeDrawer = (product: (typeof products)[number]) => {
  ...
  return (
    <main className="min-h-screen bg-[#f9e6d7] text-[#24635b]">
      ...
      <header className="sticky top-0 z-30 border-b border-black/10 bg-[#f9e6d7]/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-5 py-5 lg:px-10">
          ...
          <div className="flex items-center gap-4">
            ...
            <button aria-label="Abrir carrito" className="relative" onClick={() => setCartOpen(true)}><ShoppingBag className="size-5" />{cartCount > 0 && <span className="absolute -right-2 -top-2 flex size-4 items-center justify-center rounded-full bg-[#24635b] text-[9px] text-white">{cartCount}</span>}</button>
          </div>
        </div>
      </header>
      ...
      <section id="productos" className="mx-auto max-w-[1400px] px-5 py-12 lg:px-10 lg:py-20"><div className="mb-8 flex flex-col justify-between gap-5 border-b border-black/15 pb-5 sm:flex-row sm:items-end"><div><h2 className="mt-2 text-4xl tracking-[-0.05em]">Lo nuevo</h2></div><div className="flex items-center gap-5"><div className="hidden items-center gap-4 text-xs sm:flex">{['Todo', 'Remeras', 'Pantalones'].map((item) => <button key={item} onClick={() => setCategory(item)} className={category === item ? 'border-b border-black pb-1' : 'text-black/45'}>{item}</button>)}</div><button className="flex items-center gap-2 text-xs uppercase tracking-[0.12em]"><SlidersHorizontal className="size-4" /> Filtrar</button></div></div><div className="grid grid-cols-2 gap-x-3 gap-y-10 md:grid-cols-4 md:gap-x-5">{visibleProducts.map((product, index) => <article key={product.name} className="group"><div className="relative aspect-[0.8] overflow-hidden rounded-[1.5rem] bg-[#e9e7e1]"><img src={product.image} alt={product.name} className="size-full object-cover object-center transition-transform duration-500 group-hover:scale-105" />{product.tag && <span className="absolute left-3 top-3 bg-[#f9e6d7] px-2 py-1 text-[9px] uppercase tracking-[0.14em]">{product.tag}</span>}<button aria-label={`Agregar ${product.name} a favoritos`} className="absolute right-3 top-3 rounded-full bg-[#f9e6d7]/85 p-2 opacity-0 transition-opacity group-hover:opacity-100"><Heart className="size-4" /></button><button onClick={() => openSizeDrawer(product)} className="absolute bottom-3 left-3 right-3 rounded-full bg-[#f9e6d7] py-3 text-[11px] font-medium uppercase tracking-[0.08em] opacity-100 transition-opacity sm:opacity-0 sm:group-hover:opacity-100">+ Agregar</button></div><div className="pt-3"><div className="flex justify-between gap-2 text-sm"><h3>{product.name}</h3><span>{formatPrice(product.price)}</span></div><p className="mt-1 text-xs text-black/50">{product.category} · {product.color}</p></div></article>)}</div></section>
      ...
      {sizeDrawerProduct && <div className="fixed inset-0 z-50 bg-black/30" onClick={() => setSizeDrawerProduct(null)}><section role="dialog" aria-modal="true" aria-labelledby="size-drawer-title" className="absolute bottom-0 left-0 right-0 rounded-t-[2rem] bg-[#f9e6d7] p-5 shadow-2xl sm:left-auto sm:max-w-md" onClick={(event) => event.stopPropagation()}><div className="mx-auto mb-5 h-1 w-10 rounded-full bg-[#24635b]/25" /><div className="flex items-start gap-4"><img src={sizeDrawerProduct.image} alt={sizeDrawerProduct.name} className="size-24 rounded-2xl object-cover" /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-3"><div><h2 id="size-drawer-title" className="text-xl leading-none tracking-[-0.03em]">{sizeDrawerProduct.name}</h2><p className="mt-2 text-xs text-black/55">{sizeDrawerProduct.category} · {sizeDrawerProduct.color}</p></div><button aria-label="Cerrar selector de talle" onClick={() => setSizeDrawerProduct(null)}><X className="size-5" /></button></div><p className="mt-3 text-lg">{formatPrice(sizeDrawerProduct.price)}</p></div></div><div className="mt-7"><div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-medium">Elegí tu talle</h3><button className="text-xs text-black/55 underline underline-offset-4">Guía de talles</button></div><div className="grid grid-cols-5 gap-2">{['S', 'M', 'L', 'XL', 'XXL'].map((size) => <button key={size} aria-pressed={selectedSize === size} onClick={() => setSelectedSize(size)} className={`rounded-full py-3 text-xs transition-colors ${selectedSize === size ? 'bg-[#24635b] text-[#f9e6d7]' : 'bg-white/60 text-[#24635b] hover:bg-white'}`}>{size}</button>)}</div></div><button onClick={() => { addToCart(); setSizeDrawerProduct(null) }} className="mt-6 w-full rounded-full bg-[#24635b] py-4 text-xs font-medium uppercase tracking-[0.08em] text-[#f9e6d7]">Agregar talle {selectedSize}</button></section></div>}
