...
import { addProductVariant, updateProduct } from '@/features/backoffice/actions/products'
import { ProductImageUploadForm } from '@/features/backoffice/components/product-image-upload-form'
...
export default async function EditProductPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="admin-content"><div className="admin-page-head"><div><div className="admin-kicker">CATÁLOGO / PRODUCTO</div><h1 className="admin-heading">Editar producto</h1><p className="admin-subtitle">Actualizá su información y estado de publicación.</p></div><Link href="/admin/productos" className="admin-panel-caption">← Volver</Link></div>
    ...
    <h2 className="admin-panel-title" style={{marginTop:30}}>Imágenes</h2><div className="admin-image-list">{(images??[]).map(image=><div key={image.id}><img src={supabase.storage.from('product-images').getPublicUrl(image.storage_path).data.publicUrl} alt={image.alt_text??product.name}/><span>{image.alt_text??'Sin descripción'}</span></div>)}</div><ProductImageUploadForm productId={product.id} productName={product.name} /></section></div>
}
